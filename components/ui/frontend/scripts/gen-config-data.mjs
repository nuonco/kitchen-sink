...
/* ---------- the case branches: each one's diff against main ---------- */

// The three common requests are git branches of this repo. Their deltas are
// read from git so the UI can only ever show lines the branch contains. The
// output is committed for the same reason as config-data.gen.ts: the image
// build has no repo. When a branch cannot be resolved here (a clone without
...
const base = resolveRef('origin/main', 'main')
for (const name of caseBranchNames) {
  const ref = resolveRef(name, `origin/${name}`)
  if (!base || !ref) {
    allResolved = false
  ...
  caseBranches[name] = {
    branch: name,
    branchName: branchToml.name,
    trackedBranch: branchToml.public_repo?.branch ?? branchToml.connected_repo?.branch ?? '',
    base: base.replace(/^origin\//, ''),
    baseCommit: git('rev-parse', '--short', base),
    head: git('rev-parse', '--short', ref),
    groups: (branchToml.install_groups ?? [])
      .slice()
      .sort((a, b) => a.order - b.order)
      .map((g) => ({ name: g.name, order: g.order, selector: selectorText(g.label_selector) })),
    files,
  }
}

if (allResolved) {
  const deltas = `// GENERATED by scripts/gen-config-data.mjs from git: each case branch's
...
export const caseBranches: Record<string, CaseBranch> = ${ts(caseBranches)}
`
  writeFileSync(deltasFile, deltas)
  console.log(`gen-config-data: wrote ${deltasFile}`)
} else if (!existsSync(deltasFile)) {
  console.error('gen-config-data: no case branches and no committed case-deltas.gen.ts to fall back to')
  process.exit(1)
}
