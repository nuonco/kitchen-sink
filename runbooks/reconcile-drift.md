...
1. **drift-plan** — plans the `relay` chart with `plan_only`, so the run records
...
5. **reconcile-app** — redeploys `relay` with `deploy_dependents`, which rolls
...
`certificate` → `relay` → `application_load_balancer`. Reprovisioning the sandbox
