...
2. **workload-health** — runs the existing **cron_status** action, which reports the
   `kitchen-sink` namespace's pods and services and emits `pods_ready` /
...
4. **gateway-health** — the Helm releases in the namespace and a full describe of the
   GKE Gateway and HTTPRoute, including load balancer and certificate conditions.
