...
`kitchen_sink`, `gateway`, `certificate`, `external_dns`, `pulumi_infra`, and the
actions `cron_status`, `debug`, `break_glass_remediation`.

| Runbook | Scenario | Steps |
|---------|----------|-------|
| [`full-health-check`](./full-health-check.md) | **Health check** — many signals at once, read-only | nodes → `cron_status` → rollout convergence → GKE Gateway → public endpoint |
| [`debug-bundle`](./debug-bundle.md) | **Debug** — something's gone wrong, read-only | `debug` → pod/restart detail → Gateway + synced secrets → endpoint probe |
| [`reconcile-drift`](./reconcile-drift.md) | **Drift** — re-apply desired state (applies changes) | `plan_only` chart plan → `sandbox_reprovision` → `pulumi_infra` → certificate and DNS → `kitchen_sink` + dependents → verify |
